...
import { fetchMyWishlist, removeFromWishlist } from '../../../lib/api/wishlist';
    ...
    <div>
      ...
      {!loading && entries.length === 0 && (
        <p className="text-sm text-gray-500">
          ...
          <Link href="/products" className="underline text-gray-900">
            Browse products
          </Link>
        </p>
      )}
        ...
        {entries.map((entry) => {
          ...
          return (
            <div key={entry._id} className="border border-gray-200 rounded-lg overflow-hidden flex flex-col">
              <Link href={`/products/${product._id}`} className="block w-full aspect-square bg-gray-100 overflow-hidden">
                {product.media?.[0]?.url && (
              ...
              <div className="p-4 flex-1 flex flex-col">
                <Link href={`/products/${product._id}`} className="text-sm font-medium text-gray-900 hover:underline">
                  {product.name}
                </Link>
