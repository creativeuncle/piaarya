...
function SubSection({ item, onClose }) {
  ...
  if (!hasChildren) {
    return (
      <Link href={item.route || '#'} onClick={onClose} className="block py-2 text-sm text-gray-700">
        {item.label}
      </Link>
    );
  }
      ...
      {open && (
        <div className="pl-3 pb-1 space-y-1">
          {item.children.map((leaf) => (
            <Link key={leaf.route} href={leaf.route || '#'} onClick={onClose} className="block py-1.5 text-sm text-gray-600">
              {leaf.label}
            </Link>
          ))}
        </div>
    ...
    <>
      ...
      <div className="fixed top-0 left-0 bottom-0 w-[85%] max-w-sm bg-white text-gray-900 z-50 flex flex-col">
        ...
        <div className="border-t border-gray-200 px-5 py-4">
          <Link href="/dashboard/profile" onClick={onClose} className="flex items-center gap-2 text-sm font-medium">
